...
- Hover on the order number, **Product**, and **Supplier** shows a copy icon. Only the icon copies; number click and product click both open the item-scoped `SupplierOrderModal` (enabling take-on-charge and line-level actions), while supplier click opens the supplier editor. Collapsed multi-item `N items` and child-row `—` have no icon. Spec: [UI_DESIGN_SYSTEM.md — Hover copy icon](./UI_DESIGN_SYSTEM.md#hover-copy-icon).
...
- Sale/repair card `Serials x/y` bind-modal candidate rows show the same supplier-order number (hover copy + click). Click opens this same item-scoped `SupplierOrderModal`; the bind modal stays open. Spec: [WAREHOUSE_FLOW.md §4.3.2](./WAREHOUSE_FLOW.md#432-bind-modal-candidate-row-purchase-price--supplier-order).
