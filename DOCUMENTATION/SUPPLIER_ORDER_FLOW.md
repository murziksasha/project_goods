...
- Hover on the order number, **Product**, and **Supplier** shows a copy icon. Only the icon copies; number click still opens the order, product click still opens the catalog product, supplier click still opens the supplier. Collapsed multi-item `N items` and child-row `—` have no icon. Spec: [UI_DESIGN_SYSTEM.md — Hover copy icon](./UI_DESIGN_SYSTEM.md#hover-copy-icon).
