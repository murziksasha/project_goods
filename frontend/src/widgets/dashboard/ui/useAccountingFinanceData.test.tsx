...
const mockSuccessfulApi = () => {
  vi.mocked(getCashboxes).mockImplementation(async (options?: { includeArchived?: boolean }) =>
  ...
  vi.mocked(getFinanceTransactions).mockResolvedValue([transaction()]);
  vi.mocked(getFinanceCurrencies).mockResolvedValue([currency()]);
  vi.mocked(getFinanceReport).mockResolvedValue(report());
  vi.mocked(getSupplierOrdersForPayment).mockResolvedValue([queueItem()]);
  vi.mocked(getSupplierOrders).mockResolvedValue([supplierOrder()]);
};
...
describe('useAccountingFinanceData', () => {
  ...
  it('loads finance data and applies stored cashbox order', async () => {
    ...
    expect(getCashboxes).toHaveBeenCalledWith({ includeArchived: true });
  });
  ...
  it('falls back when stored cashbox order is invalid and refreshes from events', async () => {
    ...
    await waitFor(() => expect(getFinanceReport).toHaveBeenCalledTimes(2));
  });
