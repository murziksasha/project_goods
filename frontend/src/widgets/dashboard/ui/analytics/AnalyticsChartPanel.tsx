...
const visibleSnapshots = (
  snapshots: DashboardAnalyticsView['revenueSnapshots'],
) =>
  snapshots.filter(
    (snapshot, index) => index === 0 || snapshot.total > 0,
  );
...
> = ({ analytics, isLoading }) => {
  ...
  const productValues = useMemo(
    () => analytics.productRevenueSnapshots[0]?.values ?? [],
    [analytics.productRevenueSnapshots],
  );
  const repairValues = useMemo(
    () => analytics.repairRevenueSnapshots[0]?.values ?? [],
    [analytics.repairRevenueSnapshots],
  );
  const billedValues = analytics.revenueSnapshots[0]?.values ?? [];
  const comparison = visibleSnapshots(
    analytics.revenueSnapshots,
  ).slice(1);
  ...
  const hoverLabel =
    hoverIndex != null
      ? analytics.axisLabels[hoverIndex] || String(hoverIndex + 1)
      : '';
  ...
  return (
    <section className='analytics-chart-panel'>
      ...
      <div className='chart-legend'>
        <div className='chart-legend-item'>
          ...
          <div>
            ...
            <p>
              {formatCurrencyMetric(analytics.metrics.productRevenue)}
            </p>
          </div>
        </div>
        <div className='chart-legend-item'>
          <span
            className='chart-legend-swatch'
            style={{ backgroundColor: REPAIR_COLOR }}
          />
          <div>
            <strong>{t('analytics.mix.repair')}</strong>
            <p>
              {formatCurrencyMetric(analytics.metrics.repairRevenue)}
            </p>
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        tab === 'volume' &&
        analytics.metrics.salesCount === 0 ? (
        <p className='empty-state'>
          {t('analytics.noOrdersForPeriod')}
        </p>
      ) : (
          ...
          {hoverIndex != null ? (
            <div className='analytics-chart-tooltip' role='status'>
              ...
              {tab === 'billed' ? (
                <>
                  <span>
                    {t('analytics.mix.product')}:{' '}
                    {formatCurrencyMetric(
                      productValues[hoverIndex] ?? 0,
                    )}
                  </span>
                  <span>
                    {t('analytics.mix.repair')}:{' '}
                    {formatCurrencyMetric(
                      repairValues[hoverIndex] ?? 0,
                    )}
                  </span>
                </>
              ) : (
                <>
                  ...
                  <span>
                    {t('analytics.summary.repairOrders')}:{' '}
                    {formatMetric(
                      analytics.orderSnapshots[0]?.values[
                        hoverIndex
                      ] ?? 0,
