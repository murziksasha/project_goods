      ...
      <div className='orders-toolbar'>
        <div className='orders-toolbar-left'>
          ...
          <button
            ...
            onClick={() =>
              onFilterBarOpenChange((current) => !current)
            }
          >
          ...
          {!isInformationTab ? (
            <div className='toolbar-settings' ref={columnsMenuRef}>
              <button
                ...
                onClick={() =>
                  onColumnsMenuOpenChange((current) => !current)
                }
              >
                <svg
                  ...
                  {supplierOrdersAllColumns.map((columnKey) => (
                    ...
                    >
                      <input
                        ...
                        disabled={supplierOrdersLockedColumns.includes(
                          columnKey,
                        )}
                        onChange={() =>
                      ...
                      />
                      <span>
                        {t(`orders.supplier.columns.${columnKey}`)}
                      </span>
                    </label>
                  ))}
                  <button
          ...
          {!isInformationTab ? (
            ...
            >
              <span
                className='supplier-order-star-icon'
                aria-hidden='true'
              >
                {favoritesOnly ? '★' : '☆'}
              </span>
          ...
          {!isInformationTab ? (
            <div className='orders-search-group orders-search-group-clearable'>
              <input
                ...
                placeholder={t(
                  'orders.supplier.toolbar.searchPlaceholder',
                )}
    ...
    <>
      <div className='orders-table-wrap' ref={tableWrapRef}>
          ...
          <thead>
            <tr>
              {visibleColumns.includes('number') ? (
                <th className='supplier-orders-col-number'>
                  {t('orders.supplier.columns.number')}
                </th>
              ) : null}
              {visibleColumns.includes('product') ? (
                <th className='supplier-orders-col-product'>
                  {t('orders.supplier.columns.product')}
                </th>
              ) : null}
              {visibleColumns.includes('quantity') ? (
                <th className='supplier-orders-col-quantity'>
                  {t('orders.supplier.columns.quantity')}
                </th>
              ) : null}
              {visibleColumns.includes('price') ? (
                <th className='supplier-orders-col-money'>
                  {t('orders.supplier.columns.price')}
                </th>
              ) : null}
              {visibleColumns.includes('total') ? (
                <th className='supplier-orders-col-money'>
                  {t('orders.supplier.columns.total')}
                </th>
              ) : null}
              {visibleColumns.includes('paid') ? (
                <th className='supplier-orders-col-money'>
                  {t('orders.supplier.columns.paid')}
                </th>
              ) : null}
              {visibleColumns.includes('supplier') ? (
                <th className='supplier-orders-col-supplier'>
                  {t('orders.supplier.columns.supplier')}
                </th>
              ) : null}
              {visibleColumns.includes('createdAt') ? (
                <th className='supplier-orders-col-date'>
                  {t('orders.supplier.columns.createdAt')}
                </th>
              ) : null}
              {visibleColumns.includes('createdBy') ? (
                <th className='supplier-orders-col-created-by'>
                  {t('orders.supplier.columns.createdBy')}
                </th>
              ) : null}
              {visibleColumns.includes('deliveryDate') ? (
                <th className='supplier-orders-col-date'>
                  {t('orders.supplier.columns.deliveryDate')}
                </th>
              ) : null}
              {visibleColumns.includes('status') ? (
                <th className='supplier-orders-col-status'>
                  {t('orders.supplier.columns.status')}
                </th>
              ) : null}
              {visibleColumns.includes('paymentStatus') ? (
                <th className='supplier-orders-col-payment'>
                  {t('orders.supplier.columns.paymentStatus')}
                </th>
              ) : null}
            </tr>
          </thead>
          <tbody>
            {paginatedOrders.flatMap((order) =>
              buildSupplierOrderTableRows(
                ...
                const item =
                  row.kind === 'child' || row.kind === 'single'
                    ? row.item
                    : null;
                ...
                const openOrderModal = () => {
                  if (!canViewSupplierOrders) {
                    onError(
                      t(
                        'orders.supplier.messages.errors.noViewPermission',
                      ),
                    );
                    return;
                  }
                  ...
                  >
                    {visibleColumns.includes('number') ? (
                      <td
                        ...
                        data-label={t(
                          'orders.supplier.columns.number',
                        )}
                      >
                        <div className='supplier-order-number-cell'>
                          {isParent ? (
                            <button
                              ...
                              aria-expanded={expandedOrderIds.has(
                                order.id,
                              )}
                              aria-label={
                                expandedOrderIds.has(order.id)
                                  ? t(
                                      'orders.supplier.table.collapseOrder',
                                      {
                                        id: row.id,
                                      },
                                    )
                                  : t(
                                      'orders.supplier.table.expandOrder',
                                      {
                                        id: row.id,
                                      },
                                    )
                          ...
                          {!isChild ? (
                            <button
                              ...
                              aria-label={
                                order.isFavorite === true
                                  ? t(
                                      'orders.supplier.table.unstarOrder',
                                      {
                                        id: row.id,
                                      },
                                    )
                                  : t(
                                      'orders.supplier.table.starOrder',
                                      {
                                        id: row.id,
                                      },
                                    )
                    ...
                    {visibleColumns.includes('product') ? (
                      <td
                        ...
                        data-label={t(
                          'orders.supplier.columns.product',
                        )}
                      >
                        {isParent && summary ? (
                    ...
                    {visibleColumns.includes('quantity') ? (
                      <td
                        data-label={t(
                          'orders.supplier.columns.quantity',
                        )}
                      >
                        {isParent && summary
                    ...
                    {visibleColumns.includes('paid') ? (
                      <td
                        ...
                        className={
                          !isChild &&
                          isSupplierOrderPaidAmountUnpaid(order)
                            ? 'orders-money-unpaid'
                            : undefined
                        }
                      >
                    ...
                    {visibleColumns.includes('supplier') ? (
                      <td
                        ...
                        data-label={t(
                          'orders.supplier.columns.supplier',
                        )}
                      >
                        {isChild ? (
                          notApplicableLabel
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (!matchedSupplier) {
                                  onError(
                                    t(
                                      'orders.supplier.messages.errors.supplierNotFound',
                                    ),
                                  );
                                  return;
                                }
                    ...
                    {visibleColumns.includes('createdAt') ? (
                      <td
                        data-label={t(
                          'orders.supplier.columns.createdAt',
                        )}
                      >
                        {isChild
                          ? notApplicableLabel
                    ...
                    {visibleColumns.includes('createdBy') ? (
                      <td
                        data-label={t(
                          'orders.supplier.columns.createdBy',
                        )}
                      >
                        {isChild
                          ? notApplicableLabel
                          : order.createdBy || notApplicableLabel}
                      </td>
                    ) : null}
                    {visibleColumns.includes('deliveryDate') ? (
                      <td
                        data-label={t(
                          'orders.supplier.columns.deliveryDate',
                        )}
                        className={
                          !isChild &&
                          isSupplierOrderDeliveryOverdue(order)
                    ...
                    {visibleColumns.includes('status') ? (
                      <td
                        data-label={t(
                          'orders.supplier.columns.status',
                        )}
                      >
                        {isChild ? (
                          ...
                          <div className='supplier-order-status-picker'>
                            <button
                              ...
                              data-supplier-order-status-trigger={
                                row.id
                              }
                              disabled={
                                !canManageSupplierOrders ||
                    ...
                    {visibleColumns.includes('paymentStatus') ? (
                      <td
                        data-label={t(
                          'orders.supplier.columns.paymentStatus',
                        )}
                      >
                        {isChild ? (
                          <span className='supplier-order-group-child-muted'>
        ...
        {!isLoading && paginatedOrders.length === 0 ? (
          <p className='orders-empty'>
            {t('orders.supplier.table.empty')}
          </p>
        ) : null}
        {!isLoading && paginatedOrders.length > 0 ? (
          <div className='supplier-orders-totals' role='status'>
            <span>
              {t('orders.supplier.table.totalsOrders', {
                count: totals.orderCount,
              })}
            </span>
            <span>
              {t('orders.supplier.table.totalsPcs', {
                count: totals.pcs,
              })}
            </span>
            <span>
  ...
  return createPortal(
    ...
    >
      <div className='supplier-order-status-menu-header'>
        {t('orders.supplier.statusMenu.orderLabel', {
      ...
      >
        {manualSupplierOrderStatuses.map((status) => (
          <button
            ...
            onClick={() =>
              onUpdateStatus(openStatusOrder.order, status.key)
            }
          >
            {t(status.labelKey)}
          </button>
