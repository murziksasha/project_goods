...
export interface ServiceSalePriceFieldProps {
  value: string;
  onChange: (value: string) => void;
  service: Pick<
    ServiceCatalogItem,
    'price' | 'salePriceOptions'
  > | null;
...
export const ServiceSalePriceField: React.FC<
...
> = ({
  ...
  const showTierToggle = service
    ? hasServiceWholesaleSalePrice(service)
    : false;
  ...
  const activeTier = useMemo(() => {
    ...
    if (
      priceTier &&
      matchesServiceSalePriceTier(service, value, priceTier)
    ) {
      return priceTier;
    }
  ...
  const handleTierChange = (tier: ServiceSalePriceTier) => {
    ...
    onChange(
      formatServiceSalePrice(
        getServiceSalePriceByTier(service, tier),
      ),
    );
  };
  ...
  if (tierTogglePlacement === 'compact') {
    return (
      <div
        className={
          fieldClassName
            ? `${fieldClassName} product-sale-price-field-compact`
            : 'product-sale-price-field-compact'
        }
      >
        {tierToggle ? (
  ...
  const fieldContent = (
    <div className='product-sale-price-field'>
      {stepper}
      {tierToggle && tierTogglePlacement === 'inline'
        ? tierToggle
        : null}
    </div>
  );
  ...
  return (
    <label className={fieldClassName}>
      <span className='product-sale-price-field-label'>
        <span className='product-sale-price-field-label-text'>
          {label}
        </span>
        {tierToggle && tierTogglePlacement === 'label'
          ? tierToggle
          : null}
      </span>
      {fieldContent}
